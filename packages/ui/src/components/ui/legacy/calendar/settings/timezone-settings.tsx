          ...
          ([group, tzList], idx, arr) =>
            tzList.length > 0 && (
              <div key={group} data-region-group className="py-1">
                <div className="bg-muted text-muted-foreground rounded-t-md px-2 py-1.5 text-xs font-semibold">
                  {group}
                </div>
                {tzList
                  .filter(
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <Popover open={primaryOpen} onOpenChange={setPrimaryOpen}>
          ...
          <PopoverContent
            className="min-w-(--radix-popover-trigger-width) bg-popover pointer-events-auto rounded-lg border p-0 shadow-lg"
            align="start"
            role="listbox"
            tabIndex={0}
